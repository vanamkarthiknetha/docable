...
const MenuBar = ({ editor }) => {
  ...
  return (
    <div className=" flex gap-2 mb-4 p-2 bg-black rounded-md border border-white/30 w-full">
      <button
        onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()}
        className={`px-2 ${editor.isActive('heading', { level: 1 }) ? 'bg-white/10 rounded' : ''}`}
        title="Heading 1"
      >
        H1
      </button>
      ...
      <button
        ...
        className={`px-2 ${editor.isActive('heading', { level: 3 }) ? 'bg-white/10 rounded' : ''}`}
        title="Heading 3"
      >
        H3
      </button>
      <button
        ...
        className={`px-2 ${editor.isActive('bulletList') ? 'bg-white/10 rounded' : ''}`}
        title="Bullet List"
      >
        • List
      </button>
